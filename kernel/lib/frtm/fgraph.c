...
void f_rlineto()
{
  ...
  x = pop(); EC;
#ifdef EOTA
  line_vga(cx, cy, cx+x, cy+y, color, 0);
#else
  if (! gflag) return;
...
void f_ellipsf()
{
  ...
  x = pop(); EC;
#ifdef EOTA
  fillellipse_vga(x, y, w, h, color, 0);
#else
  if (! gflag) return;
...
#else
  draw_string(x, y, str, color, font);
#endif
#else
  if (! gflag) return;
...
#else
  draw_istring(x, y, str, color, bgcolor, font);
#endif
#else
  if (! gflag) return;
  XSetFont(d, gc[color], font_dat[font].f);
...
void f_ktest()
{
  set_bitmap(100, 100, 14, 14, sizeof(ktest), ktest, color, bgcolor);
}
#endif

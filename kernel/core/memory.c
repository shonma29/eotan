...
ER get_blk(VP * pblk, ID mplid, INT blksz)
{
  ...
  retry:
    ...
    memory_pool_table[mplid].wait_tasks =
	add_tcb_list(memory_pool_table[mplid].wait_tasks, run_task);
#endif
    slp_tsk();
    goto retry;
/* NOT REACHED */
...
/* 最初に取得するブロックのサイズとメモリプールの
   各エントリのヘッダサイズとを求めておく */
    ...
    for (p = memory_pool_table[mplid].pivot->next, q =
	 memory_pool_table[mplid].pivot;
	 p != memory_pool_table[mplid].pivot; q = p, p = p->next) {
	/* メモリブロック獲得の条件：
